...
void check_aggressive_rc(int oc, int xr, int values[][5], int b[][5])
{
	for (int i = 0; i < 5; ++i)
	{
		...
		for (int p = 0; p < 5; ++p)
		{
			for (int q = 0; q < 5; ++q)
			if (b[p][q] == 1) xr++;		//row
			if (oc >= 2 && xr == max_xr && oc != 0)	values[p][i] += oc;
			xr = 0;
		}
		oc = 0;
	}
...
void check_aggressive_cr(int orow, int xc, int values[][5], int b[][5])
{
	for (int i = 0; i < 5; ++i)
	{
		...
		for (int p = 0; p < 5; ++p)
		{
			for (int q = 0; q < 5; ++q)
			if (b[q][p] == 1) xc++;		//row
			if (orow >= 2 && xc == max_xc && orow != 0)	values[i][p] += orow;
			xc = 0;
		}
		orow = 0;
	}
...
//called if there are no optimal moves
pair<int,int> coor_tie_game(int b[][5])
{
	for (int i = 0; i < 5; ++i)								//find any open
	{
		for (int j = 0; j < 5; ++j)
		{
			if (b[i][j] == 0)
